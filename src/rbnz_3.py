...
# --- replace the existing _extract() definition with this version ----------
def _extract(page_url: str,
             item_sel: str = "article, div.search-result, div.result"
             ) -> List[Tuple[str, str, str]]:
    """Open URL in head-less Chromium, extract (title, url, date)."""
    for attempt in range(1, 3):                   # ≤2 tries
        try:
            with sync_playwright() as p:
                ...
                # ❶ ───── speed-up: skip images / fonts
                page.route(
                    "**/*.{png,jpg,jpeg,gif,svg,webp,woff,woff2}",
                    lambda r: r.abort()
                )

                # ❷ ───── the only critical change → wait_until="domcontentloaded"
                print(f"Fetching {page_url}  (attempt {attempt}) …")
                page.goto(page_url,
                          wait_until="domcontentloaded",
                          timeout=120_000)   # plenty of time

                page.wait_for_selector(item_sel, timeout=60_000)
                _scroll_to_bottom(page)        # lazy-load

                items = []
                for c in page.query_selector_all(item_sel):
                    link_node = c.query_selector("h3 a, h2 a, a")
                    if not link_node:
                        continue
                    title = link_node.inner_text().strip()
                    href = (link_node.get_attribute("href") or "").strip()
                    if href.startswith("/"):
                        href = "https://www.rbnz.govt.nz" + href
                    date_node = c.query_selector(
                        "time, .published-date, .result-date, span.date")
                    iso = _clean_date(date_node.inner_text()) if date_node else ""
                    items.append((title, href, iso))
